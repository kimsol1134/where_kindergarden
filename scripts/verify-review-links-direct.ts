...
function assessDirectMatch(
  ...
  if (hasNegativeTopic && !hasFullName) {
    return {
      status: 'mismatch',
      confidence: 0.92,
      reasons: ['제목이 유치원 후기와 다른 업종/주제이고 본문에서 정식명 매칭이 약함'],
    };
  }

  if (hasFullName && hasReviewContext) {
    return {
      status: 'verified',
      confidence: 0.9,
      reasons: ['본문에서 대상 유치원 정식명과 후기성 문맥을 확인함'],
    };
  }

  if (hasCoreName && hasReviewContext) {
    return {
      status: 'verified',
      confidence: 0.76,
      reasons: ['본문에서 대상 유치원 핵심명과 후기성 문맥을 확인함'],
    };
  }
  ...
  return {
    status: 'uncertain',
    confidence: 0.45,
    reasons: ['기관명은 일부 확인되나 후기성 문맥이 약해 보류'],
  };
}
